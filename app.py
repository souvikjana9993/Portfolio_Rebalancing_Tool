...
def fetch_latest_prices(stocks):
    latest_prices = {}
    for stock in stocks:
        if stock in yfinance_symbols:
            try:
                ticker = yf.Ticker(yfinance_symbols[stock])
...
def calculate_rebalancing(holdings_df, target_ratios, extra_funds=0, allocation_margin_percent=2.0):
    if holdings_df.empty or not target_ratios:
        return [], {"status": "No Action", "amount": 0, "message": "No holdings or target ratios provided"}, holdings_df, [], {}

    target_stocks = list(target_ratios.keys())
    latest_prices = fetch_latest_prices(target_stocks)
    filtered_holdings = holdings_df[holdings_df['Instrument'].isin(target_stocks)].copy()

    if filtered_holdings.empty:
        return [], {"status": "No Action", "amount": 0, "message": "No target stocks in holdings"}, filtered_holdings, [], {}
    ...
    updated_quantities = {stock: math.floor(target_values[stock] / latest_prices.get(stock, 0)) if latest_prices.get(stock, 0) > 0 else 0 for stock in target_stocks}
    ...
    for stock in target_stocks:
        ...
        if updated_qty > original_qty:
            ...
            rebalancing_actions.append({"Instrument": stock, "Original Qty": original_qty, "Action": "Buy", "Shares": shares, "Value Bought/Sold": shares * stock_price, "Stock Price": stock_price, "New Qty": updated_qty})
        elif updated_qty < original_qty:
            shares = original_qty - updated_qty
            rebalancing_actions.append({"Instrument": stock, "Original Qty": original_qty, "Action": "Sell", "Shares": shares, "Value Bought/Sold": shares * stock_price, "Stock Price": stock_price, "New Qty": updated_qty})
    ...
    updated_holdings_df['Allocation %'] = (updated_holdings_df['Current Value'] / new_portfolio_value * 100).round(2) if new_portfolio_value > 0 else 0

    non_target_holdings = holdings_df[~holdings_df['Instrument'].isin(target_stocks)].copy()
    ...
    funds_display = {"status": "Excess Funds", "amount": available_funds, "message": f"₹{available_funds:.2f} remains unused after rebalancing."}
    tentative_holdings = [
        {"Stock": name_mapping_inv.get(row['Instrument'], row['Instrument']), "Qty": row['Qty'], "LTP": row['LTP'], "Current Value (Qty × LTP)": row['Current Value'], "Ideal Allocation %": ideal_allocations_percent.get(row['Instrument'], 0), "Actual Allocation %": row['Allocation %']}
        for _, row in updated_holdings_df.iterrows()
    ]

    return sorted(rebalancing_actions, key=lambda x: x["Instrument"]), funds_display, updated_holdings_df, sorted(tentative_holdings, key=lambda x: x["Stock"]), ideal_allocations_percent
...
with col1:
    ...
    default_ratios = {
        "BAJFINANCE": 6, "RELIANCE": 6, "KPITTECH": 5, "PIIND": 5, "TATAPOWER": 5, "KAYNES": 5, "ZOMATO": 5,
        "TEJASNET": 4.5, "STARHEALTH": 4, "IDFCFIRSTB": 4, "POLYCAB": 4, "VBL": 4, "LALPATHLAB": 4, "KALYANKJIL": 3.5
    }
    ...
    if st.button("Calculate", key="calc_button"):
        if not uploaded_files:
            ...
            holdings_df['Allocation %'] = (holdings_df['Current Value'] / total_value * 100).round(2) if total_value > 0 else 0

with col2:
    if 'holdings_df' in locals():
        st.subheader("Initial Holdings")
        st.dataframe(holdings_df.style.format({"LTP": "₹{:.2f}", "Cur_val": "₹{:.2f}", "Current Value": "₹{:.2f}", "Allocation %": "{:.2f}%"}), use_container_width=True)

        rebalancing_actions, funds_info, _, tentative_holdings, _ = calculate_rebalancing(holdings_df, user_target_ratios, extra_funds, allocation_margin_percent)

        if rebalancing_actions:
            st.subheader("Rebalancing Actions")
            actions_df = pd.DataFrame(rebalancing_actions)
            st.dataframe(actions_df.style.format({"Value Bought/Sold": "₹{:.2f}", "Stock Price": "₹{:.2f}"}), use_container_width=True)
        ...
        if tentative_holdings:
            ...
            st.dataframe(tentative_df.style.format({"LTP": "₹{:.2f}", "Current Value (Qty × LTP)": "₹{:.2f}", "Ideal Allocation %": "{:.2f}%", "Actual Allocation %": "{:.2f}%"}), use_container_width=True)